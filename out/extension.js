...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerCustomEditorProvider('stateMachineEditor.smjsonEditor', new SmjsonEditorProvider(context)));
    // コマンドパレットからの手動起動コマンド（必要であれば残す）
    context.subscriptions.push(vscode.commands.registerCommand('stateMachineEditor.start', () => {
        vscode.window.showInformationMessage('.smjsonファイルを開いてエディタを開始してください。');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('stateMachineEditor.toggleEditor', async () => {
...
class SmjsonEditorProvider {
     ...
     * ドキュメントを新しいテキストで更新する
     */
    async updateTextDocument(document, content) {
        // 1. 現在のドキュメント内容を取得
